<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:79:"/www/wwwroot/bilulanlv/application/admin20161108/view/articlecomment/index.html";i:1496111977;}*/ ?>
<!DOCTYPE html>
<html>
								...
								<table class="table table-striped table-bordered table-hover dataTables-example dataTable" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
									...
									<tbody>
										<tr class="gradeA odd">
											...
											<td class=""><strong><a href="<?php echo url('index/ArticlePage/index','id='.$vo['idd']); ?>" target="_blank"><?php echo $vo['title']; ?></a></strong></td>
											<?php if($vo['parent']==0): ?>
											<td class="">空</td>
											<?php else: if(is_array($MenArticleList) || $MenArticleList instanceof \think\Collection): $i = 0; $__LIST__ = $MenArticleList;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vm): $mod = ($i % 2 );++$i;if($vo['parent'] == $vm['id']): ?>
											...
											<td class=""><span lang="<?php echo $vo['idd']; ?>,<?php echo $vo['parent']; ?>" class="glyphicon glyphicon-trash" id="<?php echo $vo['id']; ?>" onclick="deleajax(this.id,this.lang)" style="cursor:pointer;"></span></td>
										</tr>
									</tbody>
...
<script type="text/javascript">
	function deleajax(id,lang){
			...
			//AJAX提交删除
				$.ajax({
					type:"post",
					url:'<?php echo url('./admin20161108/articlecomment/del','','',true); ?>',
	...
	function reajax(id,lang){
		layer.prompt({title: '回复评论', formType: 2}, function(text){
				...
				$.ajax({
					type:"post",
					url:'<?php echo url('./admin20161108/articlecomment/recontent','','',true); ?>',
